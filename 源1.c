...
{
	int coreObjectID;
	int* directlyDensityReachable;	//store the directly density_reachable point of corePointID  /存储corePointID中直接密度可达的点
	int reachableSize;		//the number of directly density reachable  /数直接密度可达
	int capacity;			//the current capacity of the dynamic array @directlyDensityReachable /动态数组@directlyDensityReachable的当前容量
}CoreObject;
CoreObject* coreObject_Collection;	//collectint the core_object//收集核心对象
CoreObject* coreObject;			//collected core_object//核心目标
...
}QueueNode, * QueueNodePtr;   //链表储存

typedef struct LinkQueue//链接队列
{
	QueueNodePtr front;
	QueueNodePtr rear;
}LinkQueue;
...
//sequence queue END//顺序队列结束

void Init();//初始化
void ReadData();//读取数据
double calculateDistance_BetweenTwo(int, int);//计算距离
...
/*
 * calculate distance bewteen specifed point to all others points//计算指定点到所有其他点的距离
 * and seek the directly_density_reachable of the specified point &pointID//并求出指定点pointid可达的直接密度
 * */
...
{
	char filename[200];
	sprintf_s(filename, ".//DBSCAN_cluster//cluster_%d.data", clusterID);
	FILE* fwrite;
